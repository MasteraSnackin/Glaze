...
/// The fetched books that map 1:1 onto a Glaze [Lorebook] — downloadable whole,
/// no LLM involved.
List<PublicLorebook> publicJsonBooks(List<PublicLorebook> books) =>
    books.where((b) => b.accessible && !b.isJs).toList();

/// The fetched **advanced (JS)** books: public — we have the script — but their
/// entries are generated by JavaScript, so each must be rebuilt into keyed
/// entries with the active LLM before it can be saved.
List<PublicLorebook> publicJsBooks(List<PublicLorebook> books) =>
    books.where((b) => b.isJs).toList();

/// The **closed** (private) books: neither downloadable whole nor scripted, so
/// the only way back is capturing the assembled prompt and rebuilding it.
List<PublicLorebook> closedLorebooks(List<PublicLorebook> books) =>
    books.where((b) => !b.accessible && !b.isJs).toList();

/// Everything the Public section offers, in the order it renders: plain books
/// first, then the scripted ones. This is exactly the set the section's
/// "Download all" action converts and saves.
List<PublicLorebook> downloadablePublicBooks(List<PublicLorebook> books) => [
  ...publicJsonBooks(books),
  ...publicJsBooks(books),
];
