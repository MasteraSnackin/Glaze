...
class CanonTransitionFactRefRepo {
  ...
  Future<List<CanonTransitionFactRef>> getForTransitionIds(
    ...
    return (_db.select(_db.canonTransitionFactRefs)
          ..where((row) => row.appliedCanonTransitionId.isIn(ids)))
        .get()
        .then((rows) => rows
            .map((row) => CanonTransitionFactRef(
  ...
  Future<void> insert(CanonTransitionFactRef value) => _db
      ...
      .insertOnConflictUpdate(CanonTransitionFactRefsCompanion.insert(
