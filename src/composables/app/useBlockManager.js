...
import { flushPresetSave } from '@/core/states/presetState.js';
...
export function useBlockManager({ currentPreset, editingBlockId, isEditingBlock, activeEditBlock, stashedBlocks, getBlockIcon, getPresetTokens, getPresetWeight, comparePresetEntries, closeBlockEditor }) {
    const {
        ...
        listRef: toRef(() => currentPreset.value.blocks),
        getIndex: (id) => currentPreset.value.blocks.findIndex(b => b.id === id),
        onSwap: flushPresetSave,
        itemSelector: '.prompt-block'
    });
